...
def test(model, test_loader, criterion, device, hook):
    ...
    logging.info("Testing Loss: {:.2f}, Testing Accuracy: {:.2f}%, RSME: {}".format(
        total_loss,
        100.0 * total_acc,
        rmse
    ))
...
def train(model, train_loader, validation_loader, criterion, optimizer, device, hook, epochs):
    ...
    for epoch in range(1, epochs + 1):
        for phase in ['train', 'valid']:
            ...
            for step, (inputs, labels) in enumerate(image_dataset[phase]):
                ...
                rmse = RMSELoss(preds, labels.float())
                logging.info("Images [{}/{} ({:.0f}%)] Loss: {:.2f} Accuracy: {}/{} ({:.2f}%) RMSE: {} Time: {}".format(
                            running_samples,
...
def main(args):
    ...
    model = train(model, train_loader, validation_loader, loss_criterion, optimizer, device, hook, args.epochs)
...
if __name__=='__main__':
    ...
    parser.add_argument("--num-classes", type = int, default = 5,  help = "number of items in a bin [1-5]")
